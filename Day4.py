...
for passport in input:
    splits = passport.replace(":", " ").split(" ")
    if all(elem in splits for elem in req_fields):
        valid = valid + 1
...
def validate_data(dict):
    if not (len(dict["byr"]) == 4 and (1920 <= int(dict["byr"]) <= 2002)):
        return False

    if not (len(dict["iyr"]) == 4 and (2010 <= int(dict["iyr"]) <= 2020)):
        return False

    if not (len(dict["eyr"]) == 4 and (2020 <= int(dict["eyr"]) <= 2030)):
        return False

    if len(dict["hgt"]) > 2:
        height = int(dict["hgt"][:-2])
        units = dict["hgt"][-2:]
        if units == "cm":
    ...
    valid_color = re.search(r'^#(?:[0-9a-f]{6})$', dict["hcl"])
    if not valid_color:
        return False

    valid_eye_color = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]
    if dict["ecl"] not in valid_eye_color:
        return False

    valid_pid = re.search(r'^(?:[0-9]{9})$', dict["pid"])
    if not valid_pid:
        return False
...
for passport in input:
    splits = passport.replace(":", " ").split(" ")
    if all(elem in splits for elem in req_fields):
        dict = ast.literal_eval("{'" + passport.replace(":", "':'").replace(" ", "','") + "'}")
