...
def build_mask_dict(mask):
    mask_dict = {0: [], 1: []}
    for i in range(len(mask)):
        if mask[i] != "X":
            mask_dict[int(mask[i])].append(i)

    return mask_dict


def mask_bits(bits, mask):
    for val in mask.keys():
        for add in mask[val]:
            bits[add] = val

    return "".join(map(str, bits)).replace(" ", "0")


def build_mem_dict(input):
    mask_dict = {}
    mem_dict = {}
    ...
    for entry in input:
        ...
        if command == "mask":
            mask_dict = build_mask_dict(value)
        else:
            brack_idx = splits[0].find("[")
            address = int(splits[0][brack_idx + 1 : -1])

            bits = f"{int(value):36b}"
            bits = mask_bits(list(bits), mask_dict)

            mem_dict[address] = bits

    return mem_dict


mem_dict = build_mem_dict(input)

total = 0
for mem in mem_dict.keys():
    total = total + int(mem_dict[mem], 2)
print(total)
...
def get_combs(val):
    vals = []
    val_lst = np.array(list(val))
    xs = np.where(val_lst == "X")[0]
    combs = list(itertools.product([0, 1], repeat=len(xs)))

    for comb in combs:
        new_val_lst = val_lst.copy()
        for i in range(len(comb)):
            new_val_lst[xs[i]] = comb[i]
        vals.append("".join(map(str, new_val_lst)).replace(" ", "0"))

    return vals


def build_mask_dict_float(mask):
    mask_dict = {"1": [], "X": []}
    for i in range(len(mask)):
        if mask[i] != "0":
            mask_dict[mask[i]].append(i)

    return mask_dict


def build_mem_dict_float(input):
    mask_dict = {}
    mem_dict = {}
    ...
    for entry in input:
        ...
        if command == "mask":
            mask_dict = build_mask_dict_float(value)
        else:
            bits = f"{int(value):36b}"
            brack_idx = splits[0].find("[")
            address = int(splits[0][brack_idx + 1 : -1])
            address = f"{int(address):36b}"
            msk_address = mask_bits(list(address), mask_dict)

            combs = get_combs(msk_address)
...
mem_dict = build_mem_dict_float(input)
